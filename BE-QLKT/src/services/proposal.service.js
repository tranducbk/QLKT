...
class ProposalService {
  ...
  isCellChecked(cell) {
    return cell?.value
      ? String(cell.value).toUpperCase().trim() === "X"
      : false;
  }
  ...
  parseDanhHieuRow(row, rowNumber) {
    ...
    const cccd =
      cccdValue !== null && cccdValue !== undefined
        ? this.parseCCCD(cccdValue)
        : null;
  ...
  parseThanhTichRow(row, rowNumber) {
    ...
    if (!VALID_STATUS.includes(status)) {
      throw new Error(
        `Trạng thái không hợp lệ: ${status} (chỉ chấp nhận ${VALID_STATUS.join(
          " hoặc "
        )})`
      );
    }
  ...
  async submitProposal(
    ...
    try {
      ...
      if (type === "HANG_NAM") {
        ...
        const sheetDanhHieu = workbook.getWorksheet(
          SHEET_NAMES.DANH_HIEU_HANG_NAM
        );
        if (!sheetDanhHieu) {
          throw new Error(
        ...
        const sheetThanhTich = workbook.getWorksheet(
          SHEET_NAMES.THANH_TICH_KHOA_HOC
        );
        if (!sheetThanhTich) {
          throw new Error(
        ...
        sheetNienHan.eachRow((row, rowNumber) => {
          ...
          const hccsvv_hang_ba =
            row.getCell(3).value?.toString().toUpperCase() === "X";
          const hccsvv_hang_nhi =
            row.getCell(4).value?.toString().toUpperCase() === "X";
          const hccsvv_hang_nhat =
            row.getCell(5).value?.toString().toUpperCase() === "X";
          const hcbvtq_hang_ba =
            row.getCell(6).value?.toString().toUpperCase() === "X";
          const hcbvtq_hang_nhi =
            row.getCell(7).value?.toString().toUpperCase() === "X";
          const hcbvtq_hang_nhat =
            row.getCell(8).value?.toString().toUpperCase() === "X";
      ...
      if (type === "HANG_NAM") {
        for (const item of danhHieuData) {
          try {
            ...
            console.error(
              `Warning validation error for CCCD ${item.cccd}:`,
              err.message
            );
          }
        }
      ...
      const dataDanhHieu =
        type === "HANG_NAM" && Array.isArray(danhHieuData)
          ? danhHieuData
          : null;
      const dataThanhTich =
        type === "HANG_NAM" && Array.isArray(thanhTichData)
          ? thanhTichData
          : null;
      const dataNienHan =
        type === "NIEN_HAN" && Array.isArray(nienHanData) ? nienHanData : null;
    ...
    try {
      ...
      return {
        proposals: proposals.map((p) => ({
          ...
          nguoi_de_xuat:
            p.NguoiDeXuat.QuanNhan?.ho_ten || p.NguoiDeXuat.username,
          status: p.status,
          so_danh_hieu: Array.isArray(p.data_danh_hieu)
            ? p.data_danh_hieu.length
            : 0,
          so_thanh_tich: Array.isArray(p.data_thanh_tich)
            ? p.data_thanh_tich.length
            : 0,
    ...
    try {
      ...
      for (const item of danhHieuData) {
        try {
          ...
          if (item.nhan_bkbqp && specialDecisionMapping.BKBQP) {
            soQuyetDinhBKBQP =
              soQuyetDinhBKBQP || specialDecisionMapping.BKBQP.so_quyet_dinh;
          ...
          if (item.nhan_cstdtq && specialDecisionMapping.CSTDTQ) {
            soQuyetDinhCSTDTQ =
              soQuyetDinhCSTDTQ || specialDecisionMapping.CSTDTQ.so_quyet_dinh;
          ...
          errors.push(
            `Lỗi import danh hiệu CCCD ${item.cccd}: ${error.message}`
          );
        }
      }
      ...
      for (const item of thanhTichData) {
        try {
          ...
          errors.push(
            `Lỗi import thành tích CCCD ${item.cccd}: ${error.message}`
          );
        }
      }
  ...
  async exportAllAwardsExcel(filters = {}) {
    try {
      ...
      if (don_vi_id) {
        filteredAwards = awards.filter(
          (a) => a.QuanNhan.don_vi_id === don_vi_id
        );
      }
  ...
  async importAwards(excelBuffer, adminId) {
    try {
      ...
      sheet.eachRow((row, rowNumber) => {
        ...
        const nhan_bkbqp =
          row.getCell(4).value?.toString().toUpperCase() === "X";
        const so_quyet_dinh_bkbqp =
          row.getCell(5).value?.toString().trim() || null;
        const nhan_cstdtq =
          row.getCell(6).value?.toString().toUpperCase() === "X";
        const so_quyet_dinh_cstdtq =
    ...
    try {
      ...
      if (proposal.ten_file_pdf) {
        try {
          ...
          console.warn(
            `[deleteProposal] Không thể xóa file PDF: ${fileError.message}`
          );
